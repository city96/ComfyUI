...
const CanvasResizeSymbol = Symbol();

function addCanvasWidget(node, name, opts, app) {
	const MIN_SIZE = 150;
	...
	function computeSize(size) {
		...
		// See how large each text input can be
		freeSpace -= widgetHeight;
		freeSpace /= canvas.length;

		if (freeSpace < MIN_SIZE) {
			// There isnt enough space for all the widgets, increase the size of the node
			freeSpace = MIN_SIZE;
			node.size[1] = y + widgetHeight + freeSpace * canvas.length;
			node.graph.setDirtyCanvas(true);
		}
	...
	canvas.id = "wolf-canvas"
	widget.inputEl = document.createElement("div");
	widget.inputEl.className = "comfy-multiline-input";
	widget.inputEl.style.display = "block";
	widget.inputEl.id = "screen";
	widget.inputEl.appendChild(canvas);
	...
	widget.onRemove = () => {
		...
		// Restore original size handler if we are the last
		if (!--node[CanvasSymbol]) {
			node.onResize = node[CanvasResizeSymbol];
			delete node[CanvasSymbol];
			delete node[CanvasResizeSymbol];
		}
	};
	...
	if (node[CanvasSymbol]) {
		...
		node[CanvasSymbol] = 1;
		const onResize = (node[CanvasResizeSymbol] = node.onResize);
...
export const ComfyWidgets = {
	...
	STRING(node, inputName, inputData, app) {
		...
		const hascanvas = !!inputData[1].hascanvas;

		if (multiline) {
			return addMultilineWidget(node, inputName, { defaultVal, ...inputData[1] }, app);
		} else if (hascanvas) {
			return addCanvasWidget(node, inputName, { defaultVal, ...inputData[1] }, app);
		} else {
